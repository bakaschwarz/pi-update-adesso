...
export class AdessoHubClient {
  ...
  async getModelInfo(): Promise<unknown> {
    return this.getJson("/v1/model/info");
  }

  async getUserDailyActivity(startDate: string, endDate: string): Promise<unknown> {
    const qs = new URLSearchParams({ start_time: startDate, end_time: endDate });
    return this.getJson(`/user/daily/activity?${qs.toString()}`);
  }
  ...
  private async getJson(path: string): Promise<unknown> {
    ...
    try {
      ...
      if (!/application\/json/i.test(ct)) {
        ...
        throw new AdessoHubError("invalid_json", `expected application/json, got '${ct || ""}'${txt ? "; body=" + txt : ""}`);
      }
