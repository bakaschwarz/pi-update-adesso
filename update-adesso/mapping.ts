...
export function mapModels(payload: ModelInfoItem[]): ProviderModels[] {
    ...
    for (const m of payload) {
        ...
        const input: PiModel["input"] = [
            "text",
            ...((m.model_info.supports_vision ? ["image"] : []) as ("image" | "text")[]),
        ];
        ...
        const cacheWriteCost =
            Math.round(
                (m.model_info.cache_creation_input_token_cost_above_200k_tokens ??
                    m.model_info.cache_creation_input_token_cost ??
                    0) *
                    1_000_000 *
                    100,
            ) / 100;
