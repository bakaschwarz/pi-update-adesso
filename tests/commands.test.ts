...
describe("pi-update-adesso scaffold", () => {
    ...
    it("throws when ADESSO_API_KEY is missing (env key error)", async () => {
        ...
        await expect(commands["update-adesso"].handler({}, ctx)).rejects.toThrow(/Missing ADESSO_API_KEY/);
        await expect(commands["spend"].handler({}, ctx)).rejects.toThrow(/Missing ADESSO_API_KEY/);
    });
});
